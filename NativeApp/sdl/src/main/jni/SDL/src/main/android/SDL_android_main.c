...
JNIEXPORT int JNICALL Java_org_libsdl_app_SDLActivity_nativeInit(JNIEnv* env, jclass cls, jstring array);
...
//    int len;
    ...
    const char* utf = NULL;
    if(array){
        utf = (*env)->GetStringUTFChars(env, array, 0);
        if(utf){
            argv[1] = SDL_strdup(utf);
            (*env)->ReleaseStringUTFChars(env, array, utf);
        }
    }
    argv[2] = NULL;
