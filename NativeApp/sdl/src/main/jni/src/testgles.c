...
int main(int argc, char **argv) {
    ...
    int streamVideoIdx, streamAudioIdx;
    AVCodecContext *pCodecCtx;
    AVCodecParameters *avCodecParameters;
    AVCodecParameters *avCodecAudioParameters;
...
//SDL Parameters
    ...
    avCodecParameters = pFormatCtx->streams[streamVideoIdx]->codecpar;
    avCodecAudioParameters = pFormatCtx->streams[streamAudioIdx]->codecpar;

    // Find the decoder for the video stream
    pCodec = avcodec_find_decoder(avCodecParameters->codec_id);
    aCodec = avcodec_find_decoder(avCodecAudioParameters->codec_id);
    ...
    if (aCodec) {
        LOGI("find decoder: %d", avCodecAudioParameters->codec_id);
    }
...
//    sdlRect.h = SCREEN_H;
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        ...
        if (packet->stream_index == streamVideoIdx) {
        ...
        if (packet->stream_index == streamAudioIdx) {
