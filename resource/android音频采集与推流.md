...
* 完成FFmpeg aac  与Libfdk_aac 编码器切换
* NOTICE:如果使用FFmpeg aac编码器进行编码，则需要将采样格式AM_SAMPLE_FMT_S16重采样为AM_SAMPLE_FMT_FLTP 才能成功编码，否则失败；如果使用fdk_aac则无需重采样.
* 目前遇到的问题是采集数据缓冲只能使用4096.
